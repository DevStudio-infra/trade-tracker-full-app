...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/analytics/strategy-performance?period=${period}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Failed to fetch strategy performance data", details: errorData }, { status: response.status });
    }
