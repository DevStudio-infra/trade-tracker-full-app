...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/analytics/bot-comparison?period=${period}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Failed to fetch bot comparison data", details: errorData }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error("Bot comparison API error:", error);
