...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.log(`[Bot Toggle API] POST request received for bot ${id}`);

    // Handle authentication using our utility function
    const { userId, authHeader, isAuthenticated } = await handleApiAuth(request);

    if (!isAuthenticated) {
      console.log("[Bot Toggle API] Authentication failed");
      return NextResponse.json({ success: false, message: "Unauthorized" }, { status: 401 });
    }

    console.log(`[Bot Toggle API] Authenticated as user ${userId}`);

    // Ensure the authorization header is properly formatted
    const formattedAuthHeader = fixAuthorizationHeader(authHeader);
    console.log("[Bot Toggle API] Using auth header:", formattedAuthHeader ? "Bearer [token]" : "None");

    // Forward request to backend
    console.log(`[Bot Toggle API] Forwarding toggle active request to backend: ${BACKEND_URL}/api/v1/bots/${id}/toggle-active`);
    ...
    try {
      ...
      console.error("[Bot Toggle API] Error parsing response:", e);
      return NextResponse.json({ success: false, message: "Error parsing server response" }, { status: 500 });
    }

    console.log(`[Bot Toggle API] Backend response status: ${response.status}`);

    if (!response.ok) {
      console.error("[Bot Toggle API] Error response:", data);
      return NextResponse.json({ success: false, message: data?.message || "Failed to toggle bot active state" }, { status: response.status });
    }
